...
Ce mémoire présente la conception, le développement et la validation expérimentale d'un **système intégré de maintenance prédictive de la corrosion** articulé autour de trois composants : (1) une **sonde ER (Electrical Resistance) low-cost** basée sur un fil de fer monté en pont de Wheatstone, instrumentée par un amplificateur HX711 24 bits ; (2) un **système d'acquisition IoT** à microcontrôleur ESP32 en deep sleep pulsé (10 minutes) mesurant simultanément la résistance et la température (DS18B20) ; (3) un **modèle XGBoost** entraîné en protocole *run-to-failure* (RTF) selon l'esprit de la norme ISO 13381-1, prédisant à la fois le taux de corrosion (CR, en mm/an) et la durée de vie résiduelle (RUL, en heures), avec interprétabilité par analyse SHAP.

L'environnement corrosif de référence est le **Detar Plus**, détartrant industriel commercial à base d'acide chlorhydrique (5–15 %) et d'acide phosphorique (10–30 %), générant un milieu acide mixte (pH ≈ 1) reproduisant des conditions agressives comparables aux effluents acides industriels. Le protocole comporte quatre runs RTF : deux runs de référence sans inhibiteur et deux runs avec injection de l'inhibiteur **AC PROTECT 106** (à base d'imidazoline) à concentrations croissantes (0,1 % et 0,5 % v/v).

Le coût total du prototype est inférieur à 50 000 FCFA, soit un facteur 40 à 100 moins cher qu'une instrumentation industrielle équivalente (Cosasco, Permasense, Emerson Roxar). Ce travail démontre la faisabilité d'un système de maintenance prédictive de la corrosion entièrement réalisable à partir de composants disponibles localement au Cameroun, contribuant à la démocratisation de la surveillance industrielle dans les économies émergentes d'Afrique subsaharienne.

**Mots-clés :** corrosion ; maintenance prédictive ; sonde ER ; XGBoost ; durée de vie résiduelle ; ESP32 ; HX711 ; IoT ; inhibiteur de corrosion ; imidazoline ; pipeline Oil & Gas ; COTCO.
...
This thesis presents the design, development and experimental validation of an **integrated corrosion predictive-maintenance system** built on three components: (1) a **low-cost ER (Electrical Resistance) probe** based on an iron wire mounted in a Wheatstone bridge instrumented by a 24-bit HX711 amplifier; (2) an **IoT acquisition system** using an ESP32 microcontroller in pulsed deep-sleep mode (10 minutes) measuring resistance and temperature (DS18B20) simultaneously; (3) an **XGBoost model** trained under a *run-to-failure* (RTF) protocol following ISO 13381-1, predicting both corrosion rate (CR, in mm/year) and remaining useful life (RUL, in hours), with SHAP-based interpretability.

The reference corrosive medium is **Detar Plus**, a commercial industrial descaler containing hydrochloric acid (5–15 %) and phosphoric acid (10–30 %), producing a mixed-acid medium (pH ≈ 1) reproducing aggressive conditions similar to those of industrial acidic effluents. The protocol comprises four RTF runs: two baseline runs without inhibitor and two runs with injection of **AC PROTECT 106** corrosion inhibitor (imidazoline-based) at increasing concentrations (0.1 % and 0.5 % v/v).

The total prototype cost is below 50,000 FCFA, i.e., 40 to 100 times cheaper than comparable industrial instrumentation (Cosasco, Permasense, Emerson Roxar). This work demonstrates the feasibility of a corrosion predictive-maintenance system entirely built from components available on the local Cameroonian market, contributing to the democratization of industrial monitoring in sub-Saharan emerging economies.

**Keywords:** corrosion; predictive maintenance; ER probe; XGBoost; remaining useful life; ESP32; HX711; IoT; corrosion inhibitor; imidazoline; oil & gas pipeline; COTCO.
...
Le présent mémoire s'inscrit à la jonction de ces deux dynamiques. Il a pour objectif de concevoir, développer et valider expérimentalement un système intégré de maintenance prédictive de la corrosion combinant une sonde ER low-cost basée sur un microcontrôleur ESP32 et un amplificateur HX711, et un modèle d'apprentissage automatique XGBoost capable de prédire à la fois le taux de corrosion (CR, en mm/an) et la durée de vie résiduelle (RUL, en heures) d'un élément métallique exposé à un milieu acide multi-composant. Le contexte applicatif est celui des infrastructures pétrolières camerounaises, et plus particulièrement celui de l'opérateur **Cameroon Oil Transportation Company (COTCO)**, exploitant le pipeline Tchad-Cameroun depuis 2003 (COTCO, 2024 ; Wikipedia, 2024).

Après une revue de la littérature sur la corrosion, ses mécanismes, les méthodes de surveillance et les modèles prédictifs, ainsi qu'une présentation du contexte industriel et de la problématique (**Chapitre I**), nous aborderons les fondements techniques du prototype développé, les matériels mobilisés, et la méthodologie retenue pour l'acquisition et le traitement des données ainsi que pour l'entraînement du modèle d'apprentissage automatique (**Chapitre II**). Enfin, les résultats expérimentaux issus du prototype seront présentés, discutés au regard de la littérature, et confrontés aux objectifs initiaux de l'étude (**Chapitre III**).
...
Concevoir, développer et valider expérimentalement un **système intégré de maintenance prédictive de la corrosion** combinant une sonde ER low-cost à base d'ESP32 et HX711, et un modèle d'apprentissage automatique XGBoost capable de prédire simultanément le taux de corrosion (CR, en mm/an) et la durée de vie résiduelle (RUL, en heures) d'éléments métalliques exposés à un milieu corrosif acide multi-composant représentatif des conditions industrielles.

### I.3.2. Objectifs spécifiques

Trois objectifs spécifiques (OS), chronologiquement ordonnés et logiquement articulés, structurent ce travail :

- **OS1 — Concevoir et valider métrologi quement la chaîne d'acquisition ER low-cost.** Il s'agit de réaliser un capteur de résistance électrique (ER) basé sur un pont de Wheatstone, un amplificateur HX711 24 bits et un microcontrôleur ESP32 en deep sleep pulsé, capable de mesurer des variations de résistance d'un fil de fer à ±0,01 mΩ près dans un milieu acide concentré (pH ≈ 1), avec une période de mesure de 10 minutes et une autonomie compatible avec un déploiement de plusieurs jours.

- **OS2 — Entraîner et valider un modèle XGBoost à double sortie (CR + RUL).** Il s'agit, à partir des séries temporelles collectées en protocole *run-to-failure* sur quatre cycles expérimentaux de corrosion, de construire un modèle d'apprentissage automatique XGBoost prédisant à la fois le taux de corrosion instantané (en mm/an) et la durée de vie résiduelle (en heures), avec une erreur relative inférieure à 15 % (RMSE), validé par une procédure de validation croisée temporelle (TimeSeriesSplit) respectant la causalité, et interprété par analyse SHAP.

- **OS3 — Évaluer l'efficacité d'un inhibiteur industriel (AC PROTECT 106) et définir un système d'alertes graduées.** Il s'agit de quantifier expérimentalement l'effet de l'AC PROTECT 106 (inhibiteur à base d'imidazoline) sur le taux de corrosion mesuré, à deux concentrations différentes (0,1 % et 0,5 % v/v), de détecter par algorithme *changepoint* le temps d'adsorption de l'inhibiteur, et de définir un système d'alertes à trois niveaux (vert / orange / rouge) avec recommandations de dosage exploitables industriellement.
...
- **QR3 :** Dans quelle mesure les sorties du modèle XGBoost permettent-elles de calibrer un système d'alertes graduées (vert / orange / rouge) corrélées à l'efficacité expérimentale d'un inhibiteur industriel à base d'imidazoline (AC PROTECT 106), et de fournir des recommandations de dosage exploitables ?
...
Le **Chapitre I (Contexte et problématique)** établit le cadre théorique, normatif et industriel de la recherche selon les six approches du protocole de recherche, formule la problématique, les objectifs et les questions de recherche, expose l'importance de l'étude, et présente une revue détaillée de la littérature sur la corrosion, ses mécanismes, ses méthodes de surveillance et ses modèles prédictifs.

Le **Chapitre II (Outils et méthodes)** présente le prototype développé (sonde ER + acquisition IoT ESP32 + pipeline ML), l'ensemble des matériels mobilisés, les méthodes d'acquisition et de traitement des données, la méthodologie d'entraînement du modèle XGBoost (validation croisée temporelle, hyperparamètres, interprétabilité SHAP), le protocole expérimental *run-to-failure* en quatre runs, et le tableau synoptique de la démarche méthodologique.

Le **Chapitre III (Résultats et discussions)** présente les résultats expérimentaux issus du prototype, les performances métrologi ques de la sonde ER, les résultats des quatre runs RTF, les métriques de validation du modèle XGBoost, l'analyse SHAP des variables d'influence, l'évaluation de l'efficacité de l'inhibiteur AC PROTECT 106, et la discussion comparative des résultats au regard de la littérature.

La **Conclusion générale** synthétise les résultats des trois objectifs spécifiques, dresse le bilan des contributions, identifie les limites du travail et formule des recommandations pour COTCO et pour les travaux futurs.
...
L'**AC PROTECT 106** retenu dans ce travail est commercialisé comme inhibiteur à base d'imidazoline pour l'industrie pétrolière, avec une efficacité fabricant déclarée > 90 % en milieu HCl 1 N.

### I.7.8. Synthèse de la revue de littérature
...
| Inhibiteur | Protection fil | AC PROTECT 106 (imidazoline) |
...
| Run 3 | 24–72 h jusqu'à rupture | Detar Plus + AC PROTECT 106 à 0,1 % v/v | Entraînement + évaluation inhibiteur |
| Run 4 | 24–72 h jusqu'à rupture | Detar Plus + AC PROTECT 106 à 0,5 % v/v | Validation |
...
**Tableau II.10 — Démarche synoptique objectifs / activités / méthodes / résultats attendus**

| Objectif Spécifique | Activités à réaliser | Méthodes / Outils | Justifications / Résultats attendus |
|---|---|---|---|
| **OS1** — Concevoir et valider la sonde ER low-cost | (i) Câblage du pont de Wheatstone ; (ii) Programmation du firmware ESP32 (deep sleep, HX711, DS18B20) ; (iii) Tests de résolution sur résistances étalons ; (iv) Validation de la stabilité en milieu Detar Plus | Pont de Wheatstone (*R*₁ = *R*₂ = 10 Ω, *R*_REF = 0,5 Ω, *R*_SERIE = 100 Ω) ; HX711 gain 128 ; ESP32 deep sleep 600 s ; CSV série 115 200 baud ; Multimètre de précision pour étalonnage | Sonde fonctionnelle avec résolution ≤ 0,01 mΩ, stabilité ±0,5 mV sur 24 h, format CSV conforme — preuve de la faisabilité d'une chaîne low-cost ER |
| **OS2** — Entraîner et valider le modèle XGBoost (CR + RUL) | (i) Collecte de 4 runs RTF ; (ii) Nettoyage IQR + Savitzky-Golay ; (iii) Compensation thermique ; (iv) Feature engineering (10 variables) ; (v) Walk-forward CV (TimeSeriesSplit n=4) ; (vi) Entraînement XGBoost ; (vii) Évaluation MAE / RMSE / R² ; (viii) Interprétabilité SHAP | Python : Pandas, SciPy, XGBoost, Scikit-learn, SHAP ; Hyperparamètres : *n*=500, depth=4, lr=0,05, L1=0,1, L2=1,0 | *R²* > 0,70 et *RMSE* < 15 % pour CR ; RUL extrapolé cohérent avec runs 1–3 ; SHAP : top 3 variables identifiées et physiquement plausibles |
| **OS3** — Évaluer l'inhibiteur AC PROTECT 106 et calibrer le système d'alertes | (i) Runs 3 et 4 avec AC PROTECT 106 (0,1 % et 0,5 % v/v) ; (ii) Détection changepoint du temps d'adsorption ; (iii) Calcul Δ*CR* (avec/sans inhibiteur) ; (iv) Calcul efficacité η ; (v) Calibration des seuils vert/orange/rouge ; (vi) Définition recommandations de dosage | Pipeline Python : algorithme changepoint sur fenêtre glissante ; Comparaison runs inhibés vs non-inhibés ; Calibration sur distributions empiriques | Seuils d'alerte calibrés (CR=1, 5 mm/an ; RUL=12, 48 h) ; Efficacité d'inhibition η mesurée (%) — comparée à fabricant ; Recommandation de dose (0, 0,1, 0,5, 1,0 % v/v) |

L'usage des outils suit une chronologie stricte des actions pour atteindre chaque objectif spécifique : conception matérielle → acquisition expérimentale → traitement et modélisation → validation et calibration.

---

## II.8. Conclusion du Chapitre II

Ce chapitre a présenté l'ensemble des outils et de la méthodologie retenus pour répondre aux trois objectifs spécifiques du mémoire. Le prototype de sonde ER (pont de Wheatstone + HX711 + ESP32 en deep sleep pulsé) a été décrit dans son principe physique et son implémentation matérielle. Les matériels électroniques, chimiques et logiciels mobilisés ont été consolidés en tableaux récapitulatifs. Les méthodes d'acquisition (CSV à 10 minutes), de nettoyage (IQR + Savitzky-Golay), de compensation thermique (Matthiessen), de feature engineering (10 variables), d'entraînement XGBoost (TimeSeriesSplit, hyperparamètres) et d'interprétabilité (SHAP) ont été détaillées. Le protocole expérimental *run-to-failure* en quatre runs et le tableau synoptique de la démarche ont été présentés. Le **Chapitre III** présente à présent les résultats issus de la mise en œuvre de cette méthodologie, leur analyse et leur discussion.
...
III.4. Évaluation de l'inhibiteur AC PROTECT 106 et calibration des alertes (OS3)
III.5. Discussions
III.6. Conclusion
...
> **Ce chapitre sera complété au fur et à mesure de la collecte des données expérimentales sur le prototype.** L'ensemble des sections suivantes a été pré-structuré conformément à la méthodologie présentée au Chapitre II. Chaque section est accompagnée d'une indication précise de la nature des données et des analyses à insérer.

Dans ce chapitre, il sera question de présenter (i) les résultats de la validation métrologi que de la sonde ER (objectif OS1), (ii) les données expérimentales issues des quatre runs RTF avec leurs indicateurs clés (OS2), (iii) les performances du modèle XGBoost prédisant CR et RUL ainsi que l'analyse SHAP des variables d'influence (OS2), (iv) l'évaluation de l'efficacité de l'inhibiteur AC PROTECT 106 et la calibration du système d'alertes (OS3), puis de discuter ces résultats au regard des objectifs initiaux et de la littérature internationale (Chapitre I).
...
### III.4.1. Détection du temps d'adsorption (changepoint)
...
| Concentration | η mesurée (%) | η fabricant (%) | Écart (%) |
|---|---|---|---|
| 0,1 % v/v | … | > 90 (à vérifier) | … |
| 0,5 % v/v | … | > 90 | … |

*Discussion des écarts au regard de la composition Detar Plus (HCl + H₃PO₄) vs HCl pur des essais fabricant.]*

### III.4.3. Calibration finale des seuils vert / orange / rouge
...
### III.5.1. Performance de la sonde ER low-cost vs systèmes commerciaux
...
### III.5.2. Performance du modèle XGBoost — confrontation à la littérature
...
*Discuter les raisons d'éventuels écarts (taille du jeu de données, type de milieu Detar Plus vs corrosion CO₂, protocole RTF).]*

### III.5.3. Effet de l'inhibiteur AC PROTECT 106 — interprétation physico-chimique
...
*[À compléter après acquisition des données — Synthétiser les résultats des trois objectifs spécifiques : (i) validation métrologi que de la sonde ER (résolution mΩ obtenue, stabilité), (ii) performance XGBoost (R², RMSE pour CR et RUL), (iii) évaluation de l'inhibiteur AC PROTECT 106 (efficacité η mesurée, seuils calibrés). Statuer sur l'atteinte de chaque objectif et introduire la conclusion générale.]*
...
- **OS2 — Modèle XGBoost à double sortie :** le pipeline complet (nettoyage IQR + Savitzky-Golay, compensation thermique, feature engineering à 10 variables, validation croisée walk-forward, hyperparamètres XGBoost, interprétabilité SHAP) a été implémenté en Python et est prêt à recevoir les données expérimentales. *[Les métriques R² et RMSE pour CR et RUL seront ajoutées après les runs.]*

- **OS3 — Évaluation inhibiteur et alertes :** le protocole d'évaluation de l'AC PROTECT 106 à deux concentrations (0,1 % et 0,5 % v/v) a été défini, ainsi que l'algorithme de détection du temps d'adsorption (changepoint) et le système d'alertes graduées à trois niveaux (vert / orange / rouge). *[Les valeurs numériques d'efficacité η et la calibration finale des seuils seront ajoutées après les runs.]*

**Contribution centrale de ce travail :** la démonstration que la boucle complète **mesure ER → feature engineering temporel → prédiction XGBoost → recommandation d'inhibiteur** peut être implémentée de manière fonctionnelle à partir de composants accessibles dans le contexte camerounais. Cette contribution comble plusieurs gaps identifiés dans la littérature : (i) l'absence d'études ML sur la corrosion en contexte africain, (ii) l'absence de prédiction simultanée CR + RUL, (iii) l'absence de prototypes intégrant explicitement un inhibiteur industriel et son évaluation expérimentale.
...
4. Intégrer la chaîne d'acquisition au système PI Server existant via une passerelle Modbus TCP, en attendant une éventuelle certification ATEX du prototype.
...
4. Développer un dashboard web intégrant les prédictions du modèle, les alertes et les recommandations de dosage d'inhibiteur, à destination des équipes de maintenance ;
5. Étendre l'étude à d'autres familles d'inhibiteurs (amines, phosphonates) pour comparer leurs cinétiques d'adsorption en milieu Detar Plus.
...
L'**AC PROTECT 106** est un inhibiteur de corrosion industriel à base d'imidazoline. Caractéristiques principales :
...
## Annexe F — Liste des fichiers du projet

| Fichier | Rôle |
|---|---|
| `firmware/corrosion_monitor.ino` | Firmware ESP32 |
| `pipeline/corrosion_pipeline.py` | Pipeline Python ML |
| `memoire/memoire_v3.md` | Mémoire (ce document, source Markdown) |
| `memoire/memoire_v3.docx` | Mémoire (version Word) |
| `notebooks/05_phmsa_validation.py` | Notebook PHMSA (planifié) |
